  ...
  <div class="p-6 grid gap-4">
    ...
    <UCard>
      <div class="flex items-center gap-3 flex-wrap">
        <UInput v-model="filters.keyword" placeholder="搜索标题/摘要/内容" class="w-48" @keydown.enter="pagination.page=1;load()" />
        <USelect v-model="filters.status" :items="['', 'todo', 'learning', 'done', 'reviewing']" placeholder="状态" class="w-32" @update:model-value="pagination.page=1;load()" />
        <USelect v-model="filters.difficulty" :items="['', 'basic', 'advanced', 'expert']" placeholder="难度" class="w-32" @update:model-value="pagination.page=1;load()" />
        <UInput v-model="filters.category" placeholder="分类" class="w-32" @keydown.enter="pagination.page=1;load()" />
        <UButton icon="i-lucide-search" color="primary" @click="pagination.page=1;load()">搜索</UButton>
      </div>
    </UCard>
      ...
      <div class="grid gap-1">
        <div v-for="c in cards" :key="c.id" class="flex items-center justify-between gap-2 py-2 px-2 rounded hover:bg-gray-100">
          <div class="flex items-center gap-2 flex-1 min-w-0">
            ...
            <span class="text-xs text-gray-400 ml-auto">{{ c.category }}</span>
          </div>
