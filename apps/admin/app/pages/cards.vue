...
async function onSubmit() {
  ...
  try {
    if (editId.value) {
      await $fetch(`${apiBase.value}/cards/${editId.value}`, {
        method: 'PATCH',
        body: { ...form },
      })
    } else {
    ...
    showNotice(editId.value ? '更新成功' : '创建成功')
    resetForm()
    await load()
  } catch (e: any) {
...
async function del(id: string) {
  try {
    await $fetch(`${apiBase.value}/cards/${id}`, { method: 'DELETE' })
    await load()
    showNotice('删除成功')
  } catch (e: any) {
    showNotice(e.message)
...
<template>
  <div class="p-6 grid gap-4">
    ...
    <UCard>
      <div class="flex items-center gap-3 flex-wrap">
        ...
        <USelect
          ...
          :items="['', 'todo', 'learning', 'done', 'reviewing']"
          placeholder="状态"
          class="w-32"
          @update:model-value="
            pagination.page = 1
    ...
    <UCard>
      <template #header
        ><span class="font-bold">{{ editId ? '编辑 Card' : '新增 Card' }}</span></template
      >
    ...
    <UCard>
      <template #header
        ><span class="font-bold">Card 列表 ({{ pagination.total }})</span></template
      >
      <div class="grid gap-1">
        <div
          v-for="c in cards"
          :key="c.id"
          class="flex items-center justify-between gap-2 py-2 px-2 rounded hover:bg-gray-100">
    ...
    <div
      v-if="notice"
      class="fixed bottom-6 left-1/2 -translate-x-1/2 z-50 bg-gray-900 text-white text-sm rounded-lg px-4 py-2.5 shadow-lg">
      {{ notice }}
    </div>
  </div>
</template>
