...
async function save() {
  ...
  try {
    if (editId.value)
      await $fetch(`${apiBase.value}/demos/${editId.value}`, {
        method: 'PATCH',
        body: form,
      })
...
async function del(id: string) {
  try {
    await $fetch(`${apiBase.value}/demos/${id}`, { method: 'DELETE' })
    await load()
    showNotice('删除成功')
  } catch (e: any) {
    showNotice(e.message)
...
<template>
  <div class="p-6 grid gap-4">
    ...
    <UCard>
      <template #header
        ><span class="font-bold">{{ editId ? '编辑 Demo' : '新增 Demo' }}</span></template
      >
      <div class="grid gap-2">
        ...
        <div class="flex gap-2">
          <USelect
            ...
            :items="['AI & Agent', '全栈', '前端示例', '后端示例']" />
          <USelect
            v-model="form.displayMode"
            :items="['custom-page', 'generic-runner']" />
        </div>
        <div class="flex gap-2">
    ...
    <UCard>
      <template #header
        ><span class="font-bold">列表 ({{ demos.length }})</span></template
      >
