...
const { sessionId, messages, mode, errorMessage, isRunning, systemPrompt, createSession, loadHistory, sendMessage } = useChat()
...
<template>
  <UContainer as="main" class="demo-page">
    <UPageHeader title="长对话记忆 · 压缩验证" description="测试 AI 多轮对话记忆力，达到设定轮数/Token 后自动压缩历史为摘要">
      <template #headline>
    ...
    <section class="demo-page__body">
        ...
        <UCard class="chat-card" :ui="{ body: 'p-0 sm:p-0 flex flex-col h-full' }">
          <div class="flex items-center justify-between gap-2 px-4 py-2.5 border-b border-black/5">
            <div class="flex items-center gap-2 text-sm font-semibold">
              ...
              <UBadge size="xs" color="primary" variant="soft">{{ useTokenCompress ? `~${estimatedTokens} tokens` : `轮次 ${Math.ceil(messages.length / 2)}/${maxTurns}` }}</UBadge>
            </div>
          </div>
      ...
      <aside class="demo-page__side">
        <DemoCollapsibleCard title="关键变量追踪" icon="i-lucide-eye" :default-open="true">
          ...
          <div v-else class="flex flex-wrap gap-1.5">
            <span
              ...
              :class="kw.source === 'user' ? 'bg-primary/10 text-primary' : 'bg-muted text-muted'"
            >
              {{ kw.term }}
        ...
        <DemoCollapsibleCard title="历史总结" icon="i-lucide-archive">
          ...
          :pipeline-steps="[
            { icon: 'i-lucide-message-square', label: '用户输入', detail: '消息 → Schema 校验 → 轮次计数器 +1', color: '#0f766e' },
            { icon: 'i-lucide-brain', label: '上下文注入', detail: 'buildSystemPrompt() 将压缩历史摘要注入 System Prompt', color: '#0d9488' },
            { icon: 'i-lucide-link-2', label: 'AI 调用', detail: 'model.invoke/stream → OpenAI-compatible API', color: '#6366f1' },
            { icon: 'i-lucide-scroll-text', label: '自动压缩', detail: '达 maxTurns 后调 SUMMARIZE_PROMPT 生成摘要 → 重置计数器', color: '#f59e0b' },
            { icon: 'i-lucide-archive', label: '持久化', detail: '摘要存入 summaries Map，前端展示历史总结', color: '#dc2626' },
          ]"
          :code-analysis-items="[
            { file: 'schema.ts', title: 'Zod 校验', desc: 'maxTurns 1-100 可选压缩轮数，compressByToken 预留 Token 压缩开关。' },
            { file: 'memory-chat.service.ts', title: '压缩服务', desc: 'autoSummarize() 调 AI 生成摘要 → 注入 buildSystemPrompt() → 后续对话携带压缩上下文。' },
            { file: 'system.ts', title: 'Prompts', desc: 'SUMMARIZE_PROMPT 压缩模板：提取关键事实、按主题分段、压缩至 30% 以内。' },
          ]"
          :tech-tags="[
            { name: 'Memory', desc: '上下文记忆' }, { name: 'Summarization', desc: '对话摘要' }, { name: 'Token-aware', desc: 'Token 感知' }, { name: 'SSE', desc: '流式输出' },
          ]"
        />
      </aside>
    </section>
